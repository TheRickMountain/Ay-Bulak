    ...
    public static class MInput
    {
        ...
        internal static void Update()
        {
            if (Engine.Instance.IsActive && Active)
            {
                ...
                {
                    Keyboard.UpdateNull();
                    //TODO: При открытии консоли позиция мыши сбрасывается, что делает неудобным добавление предметов
                    //Mouse.UpdateNull();
                }
                else
                {
        ...
        public class MouseData
        {
            ...
            public Vector2 UIPosition => new Vector2(CurrentState.X, CurrentState.Y);

            public Vector2 UIScaledPosition => UIPosition / Engine.GlobalUIScale;

            public Vector2 GlobalPosition => Vector2.Transform(new Vector2(CurrentState.X, CurrentState.Y), Matrix.Invert(Engine.Camera.Matrix));
