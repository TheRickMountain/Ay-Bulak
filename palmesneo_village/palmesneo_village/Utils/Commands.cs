...
namespace palmesneo_village
{
    public class Commands
    {
        ...
        public void Log(object obj, Color color)
        {
            ...
            while (RenderManager.StashDefaultFont.MeasureString(str).X > maxWidth)
            {
                int split = -1;
                for (int i = 0; i < str.Length; i++)
                {
                    if (str[i] == ' ')
                    {
                        if (RenderManager.StashDefaultFont.MeasureString(str.Substring(0, i)).X <= maxWidth)
                            split = i;
                        else
                            break;
                    }
        ...
        {
            int screenWidth = Engine.ViewportWidth;
            int screenHeight = Engine.ViewportHeight;

            RenderManager.Rect(10, screenHeight - 50, screenWidth - 20, 40, Color.Black * OPACITY);
            if (underscore)
            {
                RenderManager.StashDefaultFont.DrawText(
                    RenderManager.SpriteBatch, 
                    ">" + currentText + "_", 
                    new Vector2(20, screenHeight - 42), 
                    Color.White,
                    0, 
                    Vector2.Zero, 
                    new Vector2(Engine.GlobalUIScale));
            }
            else
            {
                RenderManager.StashDefaultFont.DrawText(
            ...
            if (drawCommands.Count > 0)
            {
                ...
                for (int i = 0; i < drawCommands.Count; i++)
                {
                    RenderManager.StashDefaultFont.DrawText(
                        RenderManager.SpriteBatch, 
                        drawCommands[i].Text, 
                        new Vector2(20, screenHeight - 92 - (30 * i)), 
        ...
        private static float ArgFloat(string arg)
        {
            try
            {
                return (float)Convert.ToDouble(arg.Replace('.', ','));
            }
            catch
            {
                return 0;
